...
export class UsersService {
  ...
  checkAuthorization(data) {
    if (data !== null) {
      ...
      if (authentication) {
        this.notificationsService.success(`:: ${ this.authResponse[0].message } ::`);
        this.authResponse = [];
      } else {
        this.notificationsService.warn(`:: ${ this.authResponse[0].message } ::`);
        this.authResponse = [];
      }
	  this.notificationsService.success(':: Authentication Successfull, Access Granted !!! ::');
    }
  }
  ...
  checkAuthentication(data) {
	...
	if (data === 'error') {
      this.notificationsService.warn(':: Authentication Failed, Access Denied ?!! ::');
    }
  }
  ...
  userLogIn(user) {
    ...
    // Make Request
    this.http.post<any>(this.userLoginURL, logInDetails, httpOptions).subscribe(data => {
      this.responseData.push(data);
      const success = this.responseData[0].success;
      if (success) {
        this.notificationsService.success(':: User Logged In Successfully ::');
  ...
  loginUserPasswordChange(user) {
    ...
    if (this.adminUser.username !== null) {
      this.http.put<any>(`${ this.loginUserPasswordChangeUrl }/${ user.token }`, userDetails, httpOptions).subscribe(data => {
        this.responseData.push(data);
        const success = this.responseData[0].success;
        if (success) {
          this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
          this.responseData = [];
        } else {
  ...
  adminUserPasswordChange(user) {
    ...
    if (this.adminUser.username !== null) {
      this.http.put<any>(`${ this.adminUserPasswordChangeUrl }/${ user.token }`, userDetails, httpOptions).subscribe(data => {
        this.responseData.push(data);
        const success = this.responseData[0].success;
        if (success) {
          this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
  ...
  forgetLoginPassword(user) {
    ...
    this.http.put<any>(this.forgetLoginPasswordUrl, userDetails, httpOptions).subscribe(data => {
      this.responseData.push(data);
      const success = this.responseData[0].success;
      if (success) {
        this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
  ...
  forgetAdminPassword(user) {
    ...
    this.http.put<any>(this.forgetAdminPasswordUrl, userDetails, httpOptions).subscribe(data => {
      this.responseData.push(data);
      const success = this.responseData[0].success;
      if (success) {
        this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
  ...
  activateLoginUser(user) {
    if (this.adminUser.username !== null) {
      this.http.post<any>(`${ this.activateLoginUserUrl }/${ user.token }`, httpOptions).subscribe(data => {
        this.responseData.push(data);
        const success = this.responseData[0].success;
        if (success) {
          this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
  ...
  activateAdminUser(user) {
    if (this.adminUser.username !== null) {
      this.http.post<any>(`${ this.activateAdminUserUrl }/${ user.token }`, httpOptions).subscribe(data => {
        this.responseData.push(data);
        const success = this.responseData[0].success;
        if (success) {
          this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
  ...
  registerLoginUser(user) {
    ...
    if (this.adminUser.username !== null) {
      // Admin Request
      this.http.post<any>(this.addLogInUserURL, logInDetails, httpOptions).subscribe(data => {
        this.responseData.push(data);
        const success = this.responseData[0].success;
        if (success) {
      ...
      // User Request
      this.http.post<any>(this.loginUserRegisterURL, logInDetails, httpOptions).subscribe(data => {
        this.responseData.push(data);
        const success = this.responseData[0].success;
        if (success) {
          this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
  ...
  registerAdminUser(user) {
    ...
    // Make Request
    this.http.post<any>(this.addAdminUserURL, logInDetails, httpOptions).subscribe(data => {
      this.responseData.push(data);
      const success = this.responseData[0].success;
      if (success) {
        this.notificationsService.success(`:: ${ this.responseData[0].message } ::`);
  ...
  upDateLoginUser(user) {
    ...
    // Update In UI
    const modifiedUsers = this.loginUsers.filter(userDetails => userDetails._id !== logInDetails._id );
  ...
  upDateAdminUser(user) {
    ...
    // Update In UI
    const modifiedUsers = this.adminUsers.filter(userDetails => userDetails._id !== logInDetails._id );
  ...
  adminLogIn(user) {
    ...
    // Make Request
    this.http.post<any>(this.adminLoginURL, logInDetails, httpOptions).subscribe(data => {
      this.responseData.push(data);
      const success = this.responseData[0].success;
      if (success) {
        this.notificationsService.success(':: Admin Logged In Successfully ::');
        const userDetail = this.responseData[0].data;
        this.adminUser = {
          firstname: userDetail.user.firstname,
          lastname: userDetail.user.firstname,
        ...
        this.notificationsService.warn(':: Admin Log In Failed, Try Again ::');
        this.responseData = [];
      }
    });
  }
